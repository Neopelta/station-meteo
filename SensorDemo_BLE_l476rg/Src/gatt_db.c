...
* Version            : V1.0.0
* Date               : Oct-2019
* Description        : Fonctions de création de la GATT DB et gestion des événements GATT.
********************************************************************************
* (C) COPYRIGHT STMicroelectronics - Firmware fourni "AS-IS"
*******************************************************************************/
...
/* Private macros ------------------------------------------------------------*/
#define HOST_TO_LE_16(buf, val)     do { (buf)[0] = (uint8_t)(val); (buf)[1] = (uint8_t)((val)>>8); } while(0)
#define HOST_TO_LE_32(buf, val)     do { (buf)[0] = (uint8_t)(val); (buf)[1] = (uint8_t)((val)>>8); (buf)[2] = (uint8_t)((val)>>16); (buf)[3] = (uint8_t)((val)>>24); } while(0)
...
/* Définition des UUID pour les services et caractéristiques */
...
/* Variables globales pour les handles */
